...
##### 2022 MLB Pitching Data ##### ----------------------------------

# Load in external MLB data from GitHub repo
MLB_2022_Metrics_URL <- ("https://github.com/nespo08/BC-2023/raw/main/data/SSW-data/GitHub_Metric_Data_2022.rds")
MLB_METRICS_DATA <- readRDS(url(MLB_2022_Metrics_URL, method = 'libcurl'))

MLB_2022_Eff_URL <- ("https://github.com/nespo08/BC-2023/raw/main/data/SSW-data/GitHub_Eff_Data_2022.rds")
MLB_EFF_DATA <- readRDS(url(MLB_2022_Eff_URL, method = 'libcurl'))
...
STATFUN <- function(DF){
  ...
  STATDF <- COL3 %>% bind_cols(., COL4, COL8, COL9, COL6, COL2, COL5,COL10, COL11, COL12, COL14)
  ...
  tab3 <- tableGrob(STATDF[1:dim(STATDF)[1], 1:13], rows = NULL, cols = c('Pitches','BF','Pitches/PA','Strikeouts','Walks','Hits','XBH','Runs','Max Velo','Ball %','Strike %','f-Strike %','Whiff %'), theme = tt)
  header2 <- tableGrob(STATDF[1,c(1:2)], rows=NULL, cols=c('Stat Line', 'Percentages'), theme = tt2) 
  ...
  jn2$layout[1:4 , c("l", "r")] <- list(c(1,10),c(9,13))
  g <- jn2
  g <- gtable_add_grob(g,grobs = rectGrob(gp = gpar(fill = NA, lwd = 3)),t = 1, b = nrow(g), l = 1, r = 13)
  g <- gtable_add_grob(g,grobs = rectGrob(gp = gpar(fill = NA, lwd =1.5)),t = 1, b = 2, l = 1, r = 13)
  g$widths <- unit(c(rep(1/ncol(df1), ncol(df1))), 'npc')
...
STATBULLPEN <- function(DF, Hand){
  ...
  #Combine all the data together
  STATDF <- COL1 %>% bind_cols(., COL2, COL10, COL11, COL12, COL14)
  #This makes the table
  df1 <- STATDF
  colours <- matrix('#FFFFFF', nrow(df1), ncol(df1))
  FACE <- matrix(1, nrow(df1), ncol(df1))
  ...
  tt2 <- ttheme_default(core = list(bg_params=list(fill='#2a2682', col = 'black', lwd = 3.5),fg_params = list(fontsize = 32,  col = 'white', fontface = 'bold')), colhead = list(bg_params=list(fill='#a9d4a1', col='black', lwd = 3.5),fg_params = list(fontsize = 40, col = 'black')))
  tab3 <- tableGrob(STATDF[1:dim(STATDF)[1], 1:(NUMPITCHES + 5)], rows = NULL, cols = c('Pitches',PITCHTYPES,'Ball%','Strike%','Glove Side\nStrike%','Arm Side\nStrike%'), theme = tt)
...
MLB_COMPS <- function(DF, Hand){
  ...
  # MLB metric data - velo, spinrate, extension
  CLEAN_MLB_METRICS <- MLB_METRICS_DATA %>%
  ...
  # MLB efficiency data
  CLEAN_MLB_EFF <- MLB_EFF_DATA %>%
    rename(TaggedPitchType = pitch_type, p_throws = pitch_hand) %>% # Fix col name to max original dataframe
    mutate(TaggedPitchType = factor(TaggedPitchType, levels = c(GPIT$TaggedPitchType))) %>%
    filter(p_throws == Hand & TaggedPitchType %in% GPIT$TaggedPitchType) %>%
    mutate(active_spin = 100*active_spin) # Change format of eff.
  ...
  COL2 <- DF %>%  mutate(TaggedPitchType = factor(TaggedPitchType, levels = c(GPIT$TaggedPitchType))) %>% group_by(TaggedPitchType, .drop = F) %>%
    summarize(VELO = sprintf('%.1f', mean(RelSpeed, na.rm = T)),
              ...
              EFF = sprintf('%.1f', mean(Efficiency, na.rm = T))) %>% 
    filter(., TaggedPitchType != 'NA')
  
  # Percentile calculations
  COL3 <- CLEAN_MLB_METRICS %>% 
    left_join(., COL2, by = c("TaggedPitchType")) %>%
    mutate(TaggedPitchType = factor(TaggedPitchType, levels = c(GPIT$TaggedPitchType))) %>% 
    group_by(TaggedPitchType, .drop = F) %>%
    summarize(MLBSPIN = ecdf(release_spin_rate)(SPIN),
              MLBVELO = ecdf(release_speed)(VELO),
              MLBEXT = ecdf(release_extension)(EXT)) %>%
    distinct(MLBSPIN, MLBVELO, MLBEXT)
    
  COL4 <- CLEAN_MLB_EFF %>%
    left_join(., COL2, by = c("TaggedPitchType")) %>%
    mutate(TaggedPitchType = factor(TaggedPitchType, levels = c(GPIT$TaggedPitchType))) %>% 
    group_by(TaggedPitchType, .drop = F) %>%
    summarize(MLBEFF = ecdf(active_spin)(EFF)) %>%
    distinct(MLBEFF)
  
  target <- c('Fastball', 'Sinker', 'Cutter', 'Curveball', 'Slider', 'Changeup', 'Splitter', 'Knuckle Ball', 'Overall')
  PLATETB <- Reduce(function(x,y) merge(x,y, by.all = c('TaggedPitchType')), list(COL1, COL2, COL3, COL4)) %>%
    arrange(match(TaggedPitchType, target)) %>% mutate_all(., ~ifelse(. %in% c('NA', 'NaN%', 'NaN'), '-', as.character(.))) %>%
    select(TaggedPitchType, VELO, MLBVELO, SPIN, MLBSPIN, EXT, MLBEXT, EFF, MLBEFF) %>%
    mutate(MLBSPIN = sprintf('%.0f', as.numeric(MLBSPIN)*100),
           MLBVELO = sprintf('%.0f', as.numeric(MLBVELO)*100),
           MLBEXT = sprintf('%.0f', as.numeric(MLBEXT)*100),
           MLBEFF = sprintf('%.0f', as.numeric(MLBEFF)*100)) %>%
    mutate(EFF = paste0(EFF, '%'))
  colours <- PLATETB %>%
    mutate(ID = row_number()) %>% 
    mutate_at(vars(-c(ID)), ~ifelse(ID%%2 == 0, '#e8f0e6', '#ffffff')) %>% 
    select(-c(ID)) %>%
    as.matrix()
  df1 <- PLATETB
...
VECTORPLOTS <- function(DF, DF2, P){
  ...
  MAGNIFY_Y <- c(YCOORD_MAX+2, YCOORD_MAX+10) # Min, max
  MARGIN <- c(0,0)
...
# Load in Yakkertech input file, which includes output SSW values from Glenn's model
...
repeat {
  ...
  TOPGROB <- STATBULLPEN(DF,P_THROWS) # Bullpen/game statistics and percentages
  TB1 <- SSW(DF, ssw_averages) # SSW table
  TB2 <- MLB_COMPS(DF, P_THROWS) # Comparison with MLB metrics
  ...
  # First page - Bullpen/game stats, percentages, SSW table, and MLB comparisons
  ...
  layout <- rbind(c(4,9,9,9,9,9,9,7),
                  ...
                  c(9,9,9,9,9,9,9,9),
                  c(3,3,3,3,3,3,3,3))
  grid.arrange(TOPGROB,TB1,TB2,CSTLOGO,
